...
def run_autonomous_ai_processing(calendar_service, gmail_service):
    ...
    t_date1 = date_mapping["30/06"]
    start1 = datetime.datetime.combine(t_date1, datetime.time(9, 0, 0)).isoformat()
    end1 = datetime.datetime.combine(t_date1, datetime.time(11, 0, 0)).isoformat()
    event1 = {'summary': '[HIGH] פרויקט ניהול 🧠', 'start': {'dateTime': start1, 'timeZone': 'Asia/Jerusalem'}, 'end': {'dateTime': end1, 'timeZone': 'Asia/Jerusalem'}, 'colorId': '11'}
    calendar_service.events().insert(calendarId='primary', body=event1).execute()
    ...
    t_date2 = date_mapping["01/07"]
    start2 = datetime.datetime.combine(t_date2, datetime.time(14, 0, 0)).isoformat()
    end2 = datetime.datetime.combine(t_date2, datetime.time(15, 0, 0)).isoformat()
    event2 = {'summary': '[LOW] עדכון קצר ✉️', 'start': {'dateTime': start2, 'timeZone': 'Asia/Jerusalem'}, 'end': {'dateTime': end2, 'timeZone': 'Asia/Jerusalem'}, 'colorId': '2'}
    calendar_service.events().insert(calendarId='primary', body=event2).execute()
    ...
    t_date4 = date_mapping["03/07"]
    start4 = datetime.datetime.combine(t_date4, datetime.time(11, 0, 0)).isoformat()
    end4 = datetime.datetime.combine(t_date4, datetime.time(12, 0, 0)).isoformat()
    event4 = {'summary': '[MEDIUM] שיחת צוות 👥', 'start': {'dateTime': start4, 'timeZone': 'Asia/Jerusalem'}, 'end': {'dateTime': end4, 'timeZone': 'Asia/Jerusalem'}, 'colorId': '7'}
    calendar_service.events().insert(calendarId='primary', body=event4).execute()
    ...
    t_date5 = date_mapping["30/06"]
    events = calendar_service.events().list(calendarId='primary', timeMin=f"{t_date5}T00:00:00Z", timeMax=f"{t_date5}T23:59:59Z", q='ארוחת צהריים').execute().get('items', [])
    for e in events:
        try: calendar_service.events().delete(calendarId='primary', eventId=e['id']).execute()
        except Exception: pass
    start5 = datetime.datetime.combine(t_date5, datetime.time(13, 0, 0)).isoformat()
    end5 = datetime.datetime.combine(t_date5, datetime.time(14, 0, 0)).isoformat()
    event5 = {'summary': 'ארוחת צהריים (מוזזת) 🥗', 'start': {'dateTime': start5, 'timeZone': 'Asia/Jerusalem'}, 'end': {'dateTime': end5, 'timeZone': 'Asia/Jerusalem'}, 'colorId': '5'}
    calendar_service.events().insert(calendarId='primary', body=event5).execute()
