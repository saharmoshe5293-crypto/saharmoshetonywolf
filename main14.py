...
def analyze_burnout_and_report(calendar_service, gmail_service):
    ...
    busy_days_count = sum(1 for h in daily_hours.values() if h >= 5.0)
    ...
    avg_utilization = (avg_hours / 10.0) * 100 
    ...
    plt.bar(daily_hours.keys(), daily_hours.values(), color='#34A853', edgecolor='#137333')
...
להלן דו''ח אנליטיקה מתקדמת המבוסס על שעות העבודה המוגדרות (08:00 - 18:00) לטווח המורחב (21/06 - 11/07):
